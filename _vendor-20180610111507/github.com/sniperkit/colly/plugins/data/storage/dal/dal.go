...
// DAL represents a Data Abstraction Layer
type DAL string

const (
	// (default) GORM backend (sqlite3,mysql,postgres,mssql)
	GORM Backend = "gorm" //
	// (experimental) PIVOT backend (sqlite3, tidedb, elastic, mongodb, mysql, postgres)
	PIVOT Backend = "pivot" //
)

var (
	// ErrBackendNotSupported is thrown when the Data abstraction librairy is not supported by dal
	ErrBackendNotSupported = errors.New("Data abstraction library not supported yet, please choose one of")
	// ErrCallNotSupported is thrown when a method is not implemented/supported by the current backend
	ErrCallNotSupported = errors.New("The current call is not supported with this data abstraction library")
